package com.vibelife.paper.parcels;
...
/**
 * Intercepts block place/break events and checks VibeLife parcel permissions.
 *
 ...
 *
 * Players with the vibelife.admin permission bypass all parcel checks.
 */
public class ParcelListener implements Listener {
    ...
    public ParcelListener(VibeLifePlugin plugin, ParcelManager parcelManager, LoginListener loginListener) {
        this.plugin = plugin;
    ...
    private boolean checkPermission(Player player, Location location) {
        ...
        if (player.hasPermission("vibelife.admin")) {
            return true;
        }
        ...
        // We need the VibeLife accountId, not the MC UUID
        ...
        if (accountId == null) {
            player.sendMessage("\u00a7c[VibeLife] \u00a7fYou must be logged in to build.");
            return false;
        }
        ...
        if (result != null) {
            if (!result.allowed()) {
                player.sendMessage("\u00a7c[VibeLife] \u00a7f" + result.reason());
            }
        ...
        try {
            ...
            if (!allowed) {
                ...
                player.sendMessage("\u00a7c[VibeLife] \u00a7f" + reason);
            }
            return allowed;
        } catch (Exception e) {
            plugin.getLogger().warning("Parcel check failed for " + player.getName() + ": " + e.getMessage());
            // Fail open or closed? Fail closed for safety.
            player.sendMessage("\u00a7c[VibeLife] \u00a7fUnable to verify build permissions. Try again.");
            return false;
        }
    }

    /**
     * Resolve MC UUID to VibeLife accountId.
