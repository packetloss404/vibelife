...
import com.vibelife.paper.auth.LoginListener;
...
/**
 * /friends command for managing VibeLife friendships.
 *
 * Subcommands:
...
public class FriendsCommand implements CommandExecutor, TabCompleter {
    ...
    public FriendsCommand(VibeLifePlugin plugin, LoginListener loginListener) {
        this.plugin = plugin;
        ...
        plugin.getSidecarClient().get("/api/friends?token=" + token)
                .thenAccept(response -> plugin.getServer().getScheduler().runTask(plugin, () -> {
                    if (!response.has("friends") || !response.get("friends").isJsonArray()) {
                        player.sendMessage("\u00a7e[VibeLife] \u00a7fNo friends yet.");
                        return;
                    }

                    JsonArray friends = response.getAsJsonArray("friends");
                    if (friends.isEmpty()) {
                        player.sendMessage("\u00a7e[VibeLife] \u00a7fNo friends yet. Use /friends add <player>");
                        return;
                    }

                    player.sendMessage("\u00a76[VibeLife] Friends (" + friends.size() + "):");
                ...
                .exceptionally(ex -> {
                    plugin.getServer().getScheduler().runTask(plugin, () ->
                            player.sendMessage("\u00a7c[VibeLife] \u00a7fFailed to load friends."));
                    return null;
                });
        return true;
    }
    ...
    private boolean handleAdd(Player player, String token, String[] args) {
        if (args.length < 2) {
            player.sendMessage("\u00a7c[VibeLife] \u00a7fUsage: /friends add <player>");
            return true;
        }

        Player target = Bukkit.getPlayer(args[1]);
        if (target == null) {
            player.sendMessage("\u00a7c[VibeLife] \u00a7fPlayer not found or not online.");
            return true;
        }

        String targetAccountId = AccountIdCache.get(target.getUniqueId().toString());
        if (targetAccountId == null) {
            player.sendMessage("\u00a7c[VibeLife] \u00a7fThat player is not logged into VibeLife.");
            return true;
        }
        ...
        plugin.getSidecarClient().post("/api/friends", Map.of(
            ...
            if (response.has("friend")) {
                player.sendMessage("\u00a7a[VibeLife] \u00a7fFriend request sent to \u00a7e" + target.getName());
                target.sendMessage("\u00a7a[VibeLife] \u00a7e" + player.getName() + "\u00a7f sent you a friend request!");
            } else {
            ...
            plugin.getServer().getScheduler().runTask(plugin, () ->
                    player.sendMessage("\u00a7c[VibeLife] \u00a7fError sending friend request."));
            return null;
        });
        return true;
    }
    ...
    private boolean handleRemove(Player player, String token, String[] args) {
        if (args.length < 2) {
            player.sendMessage("\u00a7c[VibeLife] \u00a7fUsage: /friends remove <player>");
            return true;
        }
        ...
        plugin.getSidecarClient().post("/api/friends?_method=DELETE", Map.of(
            ...
            if (response.has("ok") && response.get("ok").getAsBoolean()) {
                player.sendMessage("\u00a7a[VibeLife] \u00a7fRemoved \u00a7e" + args[1] + "\u00a7f from friends.");
            } else {
                player.sendMessage("\u00a7c[VibeLife] \u00a7fFriend not found.");
            }
        })).exceptionally(ex -> {
            plugin.getServer().getScheduler().runTask(plugin, () ->
                    player.sendMessage("\u00a7c[VibeLife] \u00a7fError removing friend."));
            return null;
        });
        return true;
    }
        ...
        plugin.getSidecarClient().get("/api/messages/offline?token=" + token + "&limit=10")
                .thenAccept(response -> plugin.getServer().getScheduler().runTask(plugin, () -> {
                    if (!response.has("messages") || !response.get("messages").isJsonArray()) {
                        player.sendMessage("\u00a7e[VibeLife] \u00a7fNo messages.");
                        return;
                    }

                    JsonArray messages = response.getAsJsonArray("messages");
                    if (messages.isEmpty()) {
                        player.sendMessage("\u00a7e[VibeLife] \u00a7fNo offline messages.");
                        return;
                    }

                    player.sendMessage("\u00a76[VibeLife] Offline Messages (" + messages.size() + "):");
                ...
                .exceptionally(ex -> {
                    plugin.getServer().getScheduler().runTask(plugin, () ->
                            player.sendMessage("\u00a7c[VibeLife] \u00a7fFailed to load messages."));
                    return null;
                });
        return true;
    }
    ...
    private void sendUsage(Player player) {
        player.sendMessage("\u00a76[VibeLife] Friends commands:");
