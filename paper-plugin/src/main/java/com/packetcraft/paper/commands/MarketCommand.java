...
public class MarketCommand implements CommandExecutor, TabCompleter {
    ...
    public MarketCommand(PacketCraftPlugin plugin) {
        this.plugin = plugin;
    }
        ...
        plugin.getSidecarClient().get("/api/marketplace")
                .thenAccept(response -> plugin.getServer().getScheduler().runTask(plugin, () -> {
                    if (!response.has("listings") || !response.get("listings").isJsonArray()) {
                        player.sendMessage("\u00a7e[PacketCraft] \u00a7fNo marketplace listings.");
                        return;
                    }

                    JsonArray arr = response.getAsJsonArray("listings");
                    if (arr.isEmpty()) {
                        player.sendMessage("\u00a7e[PacketCraft] \u00a7fNo active listings. Press \u00a7eM\u00a7f to open the marketplace GUI.");
                        return;
                    }

                    int max = Math.min(arr.size(), 10);
                    player.sendMessage("\u00a76[PacketCraft] Marketplace (" + arr.size() + " listings):");
                    for (int i = 0; i < max; i++) {
                ...
                .exceptionally(ex -> {
                    plugin.getServer().getScheduler().runTask(plugin, () ->
                            player.sendMessage("\u00a7c[PacketCraft] \u00a7fFailed to load marketplace."));
                    return null;
                });
