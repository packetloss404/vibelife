...
/**
 * /parcel command for managing PacketCraft parcels in-game.
 *
 * Subcommands:
...
public class ParcelCommand implements CommandExecutor, TabCompleter {
    ...
    public ParcelCommand(PacketCraftPlugin plugin, ParcelManager parcelManager, LoginListener loginListener) {
        this.plugin = plugin;
    ...
    private boolean handleInfo(Player player) {
        ...
        if (parcel == null) {
            player.sendMessage("\u00a7e[PacketCraft] \u00a7fNo parcel at your location.");
            return true;
        }

        player.sendMessage("\u00a76[PacketCraft] Parcel: \u00a7f" + parcel.name);
    ...
    private boolean handleClaim(Player player) {
        ...
        ParcelManager.ParcelData parcel = parcelManager.getParcelAt(regionId, x, z);
        if (parcel == null) {
            player.sendMessage("\u00a7c[PacketCraft] \u00a7fNo parcel at your location.");
            return true;
        }

        if (parcel.ownerAccountId != null) {
            player.sendMessage("\u00a7c[PacketCraft] \u00a7fThis parcel is already claimed.");
            return true;
        }
                ...
                .thenAccept(response -> {
                    if (response.has("parcel")) {
                        plugin.getServer().getScheduler().runTask(plugin, () -> {
                            player.sendMessage("\u00a7a[PacketCraft] \u00a7fParcel \u00a7e" + parcel.name + "\u00a7f claimed!");
                            // Refresh cache
                            parcelManager.syncRegion(regionId);
                        });
                    } else {
                        plugin.getServer().getScheduler().runTask(plugin, () ->
                                player.sendMessage("\u00a7c[PacketCraft] \u00a7fFailed to claim parcel.")
                        );
                    }
                })
                .exceptionally(ex -> {
                    plugin.getServer().getScheduler().runTask(plugin, () ->
                            player.sendMessage("\u00a7c[PacketCraft] \u00a7fError claiming parcel: " + ex.getMessage())
                    );
                    return null;
                });
        return true;
    ...
    private boolean handleRelease(Player player) {
        ...
        if (accountId == null || !accountId.equals(parcel.ownerAccountId)) {
            player.sendMessage("\u00a7c[PacketCraft] \u00a7fYou don't own this parcel.");
            return true;
        }
        ...
        plugin.getSidecarClient()
                ...
                .thenAccept(response -> {
                    if (response.has("parcel")) {
                        plugin.getServer().getScheduler().runTask(plugin, () -> {
                            player.sendMessage("\u00a7a[PacketCraft] \u00a7fParcel \u00a7e" + parcel.name + "\u00a7f released.");
                            parcelManager.syncRegion(regionId);
                        });
                    } else {
                        plugin.getServer().getScheduler().runTask(plugin, () ->
                                player.sendMessage("\u00a7c[PacketCraft] \u00a7fFailed to release parcel.")
                        );
                    }
                })
                .exceptionally(ex -> {
                    plugin.getServer().getScheduler().runTask(plugin, () ->
                            player.sendMessage("\u00a7c[PacketCraft] \u00a7fError releasing parcel: " + ex.getMessage())
                    );
                    return null;
                });
        return true;
    ...
    private boolean handleList(Player player) {
        ...
        if (parcels.isEmpty()) {
            player.sendMessage("\u00a7e[PacketCraft] \u00a7fNo parcels in this region.");
            return true;
        }

        player.sendMessage("\u00a76[PacketCraft] Parcels in region \u00a7e" + regionId + "\u00a76:");
    ...
    private void sendUsage(Player player) {
        player.sendMessage("\u00a76[PacketCraft] Parcel commands:");
