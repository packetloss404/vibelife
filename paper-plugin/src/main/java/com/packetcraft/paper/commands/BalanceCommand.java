...
/**
 * /balance - Show the player's PacketCraft currency balance.
 */
public class BalanceCommand implements CommandExecutor {

    private final PacketCraftPlugin plugin;

    public BalanceCommand(PacketCraftPlugin plugin) {
        this.plugin = plugin;
    }
        ...
        plugin.getSidecarClient().get("/api/economy/balance/" + accountId)
                .thenAccept(response -> {
                    ...
                    plugin.getServer().getScheduler().runTask(plugin, () ->
                            player.sendMessage("\u00a76[PacketCraft] \u00a7fBalance: \u00a7e" + String.format("%.0f", balance) + " Vibes")
                    );
                })
                .exceptionally(ex -> {
                    plugin.getServer().getScheduler().runTask(plugin, () ->
                            player.sendMessage("\u00a7c[PacketCraft] \u00a7fUnable to check balance.")
                    );
                    return null;
                });
