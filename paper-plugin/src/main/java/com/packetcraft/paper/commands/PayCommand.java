...
/**
 * /pay <player> <amount> - Send PacketCraft currency to another player.
 */
public class PayCommand implements CommandExecutor, TabCompleter {

    private final PacketCraftPlugin plugin;

    public PayCommand(PacketCraftPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 2) {
            player.sendMessage("\u00a7c[PacketCraft] \u00a7fUsage: /pay <player> <amount>");
            return true;
        }
        ...
        try {
            ...
            player.sendMessage("\u00a7c[PacketCraft] \u00a7fAmount must be a number.");
            return true;
        }

        if (amount <= 0) {
            player.sendMessage("\u00a7c[PacketCraft] \u00a7fAmount must be positive.");
            return true;
        }

        Player target = Bukkit.getPlayer(targetName);
        if (target == null) {
            player.sendMessage("\u00a7c[PacketCraft] \u00a7fPlayer not found.");
            return true;
        }

        if (target.equals(player)) {
            player.sendMessage("\u00a7c[PacketCraft] \u00a7fYou can't pay yourself.");
            return true;
        }
        ...
        if (fromAccountId == null || toAccountId == null) {
            player.sendMessage("\u00a7c[PacketCraft] \u00a7fBoth players must be logged in.");
            return true;
        }
        ...
        plugin.getSidecarClient()
                ...
                .thenAccept(response -> {
                    plugin.getServer().getScheduler().runTask(plugin, () -> {
                        if (response.has("success") && response.get("success").getAsBoolean()) {
                            player.sendMessage("\u00a7a[PacketCraft] \u00a7fSent \u00a7e" + amount + " Vibes\u00a7f to \u00a7e" + target.getName());
                            target.sendMessage("\u00a7a[PacketCraft] \u00a7fReceived \u00a7e" + amount + " Vibes\u00a7f from \u00a7e" + player.getName());
                ...
                .exceptionally(ex -> {
                    plugin.getServer().getScheduler().runTask(plugin, () ->
                            player.sendMessage("\u00a7c[PacketCraft] \u00a7fError processing payment.")
                    );
                    return null;
                });
